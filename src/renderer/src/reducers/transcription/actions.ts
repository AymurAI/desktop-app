...
export enum ActionTypes {
  ...
  ADD_PERSONA_SPEAKER = "ADD_PERSONA_SPEAKER",
...
export type AddPersonaSpeakerAction = Action<
  ActionTypes.ADD_PERSONA_SPEAKER,
  { transcriptionId: string; id: string }
>;
/**
 * Adds a new auto-named "Persona N" speaker to a transcription. Unlike
 * `addSpeaker`, the label/initials/color are NOT computed by the caller:
 * the reducer derives them from the transcription's OWN speakers array at
 * apply time (`nextPersonaLabel`, `computeInitials`, `SPEAKER_PALETTE`
 * indexed by the CURRENT speaker count), so N of these dispatched in the
 * same batch each see the previous one's result and produce N distinct
 * labels/colors — the bug this action exists to fix was the opposite: the
 * caller (`turn-side-panel.tsx`'s `handleNewPerson`) used to compute those
 * values itself from a stale `speakers` prop shared by every handler in the
 * batch.
 *
 * `id` is still generated by the CALLER (`crypto.randomUUID()`), not here:
 * the caller needs it immediately to dispatch `reassignTurnSpeaker`, and
 * generating it inside the reducer wouldn't return it to the caller and
 * would make the reducer impure.
 * @param transcriptionId ID of the transcription to modify
 * @param id ID for the new speaker
 */
export function addPersonaSpeaker(
  transcriptionId: string,
  id: string,
): AddPersonaSpeakerAction {
  return {
    type: ActionTypes.ADD_PERSONA_SPEAKER,
    payload: { transcriptionId, id },
  };
}
